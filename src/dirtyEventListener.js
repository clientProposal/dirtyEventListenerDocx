...
listen(documentViewer, 'documentLoaded', () => {
  ...
  onSignal?.('office listeners wired', { 
    docHasAddEventListener: !!doc?.addEventListener,
    editorHasAddEventListener: !!officeEditor?.addEventListener 
  });
});

  const spreadsheetManager = documentViewer?.getSpreadsheetEditorManager?.();
  if (spreadsheetManager?.addEventListener) {
    listen(spreadsheetManager, 'spreadsheetEditorReady', () => {
      onSignal?.('spreadsheetEditorReady — wiring sheetChanged');
      listen(spreadsheetManager, 'sheetChanged', () => {
        onSignal?.('sheetChanged');
        markDirty('sheetChanged');
      });
    });
  }
